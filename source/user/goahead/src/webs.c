...
// replaceSpaces - replace " " with "&nbsp;" so data is formatted by browser correctly
char_t *replaceSpaces(char_t *src)
{
	static char_t tmpBuff[256];
	char_t *dst = tmpBuff;

	while (*src && (dst-tmpBuff)<(sizeof(tmpBuff)-6)) {
		if (*src == ' ') {
			src++;
			*dst++ = '&'; 
			*dst++ = 'n';
