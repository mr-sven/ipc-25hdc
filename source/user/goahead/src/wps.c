...
extern int g_Raix_WscResult;                                                    // for AP only ( STA WPS don't need this)
#endif
...
void updateWPS( webs_t wp, char_t *path, char_t *query)
{
	...
	getCurrentWscProfile(query, &result, sizeof(WSC_CONFIGURED_VALUE));
	...
	//7. WSC Status
	status = getWscStatus(query);
	websWrite(wp, T("%s\n"), getWscStatusStr(status));

	//8. WSC Result
#if defined (RTDEV_SUPPORT)
	if (!strcmp(query, "rai0"))
		websWrite(wp, T("%d\n"), g_Raix_WscResult);
	else
#endif
		websWrite(wp, T("%d\n"), g_WscResult);
...
static void WPSRaxCancel(webs_t wp, char_t *path, char_t *query)
{
	resetTimerAll();
	doSystem("kill -9 `cat /var/run/wscd.pid.ra0`");
...
void formDefineWPS(void){
	...
	websFormDefine(T("WPSRaxCancel"), WPSRaxCancel);
